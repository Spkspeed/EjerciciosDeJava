...
public class MainAsistenciasTest{
    ...
    public void testAsistencias() {
        ...
        for(Alumno alumno : aulaPrimeroA.getListaAlumnos()) {
            System.out.println("Un alumno de primero A es: " + alumno.getNacionalidad() + " " + alumno.getApellido());
        }
        ...
        for(Map.Entry<Alumno, Boolean> entry : asistencia.getAsistencias().entrySet()) {
            System.out.println("El alumno de primero A es: " + entry.getKey().getNacionalidad() + " " + entry.getKey().getApellido() + " Falto?? " + entry.getValue());
